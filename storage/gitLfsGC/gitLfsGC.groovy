...
// Use an AQL query to build a list of all files contained in a repository.
def gatherFiles(repo) {
  def quer = new JsonBuilder([type: 'file', repo: repo]).toString()
  def aql = "items.find($quer).include(\"repo\",\"path\",\"name\")"
  def results = []
  searches.aql(aql) {
    for (item in it) {
      results << item.path + '/' + item.name
    }
  }
  return results
}
...
def cleanLFS(workrepos) {
  ...
  for (repo in cfg.repositories?.entrySet()) {
    ...
    // accidentally
    def repofiles = gatherFiles(repo.key)
    try {
