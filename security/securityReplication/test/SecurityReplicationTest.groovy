...
class SecurityReplicationTest extends Specification {
  def 'put and get security replication list test'() {
    ...
    def textList = '["http://localhost:8080/artifactory",'
    textList += '"http://localhost:8081/artifactory",'
    textList += '"http://localhost:8082/artifactory"]'
    conn.outputStream.write(textFile.bytes)
    assert conn.responseCode == 200
    conn.disconnect()

    conn = new URL("${baseurl}/secRepList").openConnection()
