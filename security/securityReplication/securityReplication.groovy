...
executions {
    ...
    */
    recoverySync(httpMethod: 'POST') {
        if (goldenDB == null){
            log.debug("No goldenDB something is wrong")
            message = "No goldenDB something is wrong"
            status = 400
        } else {
            def recGoldenDB = new JsonBuilder(goldenDB)
            log.debug("ALL: recoverySync is called giving my goldenDB: ${goldenDB.toString()}")
            message = recGoldenDB
            status = 200
        }
    }
    ...
    */
    secRepDataGet(httpMethod: 'GET') { params ->
        ...
        if (recovery == true && goldenDB == null) {
            message = "[]"
            status = 200
        } else {
            if (goldenDB == null || !goldenDB) {
    ...
    */
    secRepDataPost(httpMethod: 'POST') { ResourceStreamHandle body ->
        ...
        updateDatabase(nodeExtract, goldenDB)
    ...
    */
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        ...
        if (whoami != MASTER) {
            ...
            } else if (recovery == true && goldenDB == null) {
