...
memRecovery = false
retryCount = 0
baseSnapShot = ["users":[:],"groups":[:],"permissions":[:]]
...
executions {
    ...
    getUserDB(httpMethod: 'GET'){
        def inputFile = new File(artHome, '/plugins/goldenFile.json')
        def slurped = null

        slurped = getJsonFile(inputFile)
        if (slurped == 'ERROR'){
            message = "Error the golden user file either does not exist or contains no contents"
            status = 400
            return
        } else {
            message = new JsonBuilder(slurped).toString()
            status = 200
        }
    }
    ...
    userDBSync(httpMethod: 'POST') {
        ...
        slurped = getJsonFile(secRepJson)
        if (slurped == 'ERROR'){
        ...
        syncAll(buildDiff(baseSnapShot, extract()), 
                auth, 
                'dbSync', 
                slurped, 
                secRepJson)
    ...
    secRepDataGet(httpMethod: 'GET') { params ->
        ...
        if (slurped.securityReplication.recovery == true && memRecovery == false) {
            ...
            log.debug("SLAVE: slaveExract is: ${extract().getClass()}, ${extract().toString()}")

            if (action == 'diff'){
                log.debug("SLAVE: secRepDataGet diff")
                slavePatch = buildDiff(goldenDB, extract())
            } else if (action == 'dbSync'){
                log.debug("SLAVE: secRepDataGet: dbSync")
                slavePatch = buildDiff(baseSnapShot, extract())
            } else {
...
jobs {
    ...
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        ...
        slurped = getJsonFile(targetFile)
        if (slurped == 'ERROR'){
        ...
        if (whoami != MASTER) {
            if (recovery == false){
                ...
                sleep(1000)
                disasterRecovery('SLAVE', auth, slurped, targetFile, goldenDB)
            } else {
            ...
            if (recovery == false){
                ...
                disasterRecovery('MASTER', auth, slurped, targetFile, goldenDB)
            } else {
                log.debug("MASTER: Not first time coming up, lets do some updates")
                masterPatch = buildDiff(goldenDB, extract())
                ...
                syncAll(masterPatch, auth, 'diff', slurped, targetFile)
            }
        }
    }
}
...
def syncAll(patch, auth, action, slurped, jsonFile){
...
def disasterRecovery(node, auth, slurped, jsonFile, goldenDB){
    def recoverySlave = null
    def resp = null
    def distList = slurped.securityReplication.urls
...
def bringUp(node, whoami, auth, slurped, jsonFile, goldenDB){
    if (node == 'SLAVE'){
        ...
        syncAll(dbMasterPatch, auth, 'dbSync', slurped, jsonFile)
    } else {
        log.error("ALL: invalid node ${node}")
    }
}
