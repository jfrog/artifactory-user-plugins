...
def setCronJob(cron_job){
    ...
    if (cron_job == null){
        ...
        try {
            ...
            log.error("ALL: problem getting ${jsonFile}")
            return
        }
        cron_job = slurped.securityReplication.cron_job
        log.debug("ALL: config cron job is being set at: ${cron_job}")
    }  else {
        log.debug("ALL: cron_job is already set at: ${cron_job}")
    }
    return cron_job
}
...
jobs {
    def cron_job = "*/30 * * * * ?"
    cron_job = setCronJob(cron_job)

    securityReplicationWorker(cron: cron_job) {
        MASTER = null
        DOWN_INSTANCE = []
