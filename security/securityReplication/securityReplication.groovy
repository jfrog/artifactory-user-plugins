    ...
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        ...
        if (whoami != MASTER) {
            ...
            if (slaveStartUpSync == false){
                ...
                resp = remoteCall(MASTER, auth, 'slaveDBSync', null)
                if (resp.getStatusLine().getStatusCode() != 200) {
                    throw new RuntimeException("SLAVE: slaveDBSync Failed: ${MASTER}: HTTP error code: " + resp.getStatusLine().getStatusCode())
                }
                slaveStartUpSync = true
            }
            return
        } else {
