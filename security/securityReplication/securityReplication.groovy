...
executions {
    ...
    */
    ...
    Output: None
    -----------------------------------------------------------------------------
    TODO: need to handle init time db sync
    -----------------------------------------------------------------------------
    */
    ...
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        ...
        if (whoami != MASTER) {
            ...
            if (startUpSync == false){
                ...
                def dbBigDiff = grabStuffFromSlaves(distList, dbMasterPatch, whoami, auth, 'dbSync')
                
                log.debug("MASTER: the aggragated DB is: ${dbBigDiff}")

                if (dbBigDiff.isEmpty()){
                    log.debug("MASTER: The DB's are all empty nothing to syncon init")
                    startUpSync = true
                    return
                } else {
                    def dbMergedPatch = merge(dbBigDiff)
                    log.debug("MASTER: dbMergePatch: ${dbMergedPatch.getClass()}, ${dbMergedPatch.toString()}")
        
                    //comvert tp JSON
                    def jsonDBMergedPatch = new JsonBuilder(dbMergedPatch)
                    log.debug("MASTER: jsonDBMergedPatch: ${jsonDBMergedPatch.getClass()} and ${jsonDBMergedPatch.toString()}")
                    
                    log.debug("MASTER: I gotta send the golden DB back to my slaves")
                    sendSlavesGoldenCopy(distList, whoami, auth, jsonDBMergedPatch)

                    log.debug("MASTER: masterGolden: ${masterGolden.getClass()}, dbMergedPatch: ${dbMergedPatch.getClass()}")
                    masterGolden = applyDiff(masterGolden, dbMergedPatch)
                ...
                log.debug("MASTER: Going to slaves to get stuff masterPatch: ${masterPatch.getClass()}")

                def bigDiff = grabStuffFromSlaves(distList, masterPatch, whoami, auth, 'diff')
