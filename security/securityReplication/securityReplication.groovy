...
def syncAll(distList, patch, whoami, auth, action, slurped, jsonFile){
    ...
    if (bigDiff.isEmpty()){
        ...
        if (action == 'dbSync') {
            ...
            writeToJson('MASTER', slurped, jsonFile)
        }
        return
    } else {
...
def writeToJson(node, slurped, jsonFile){
...
def disasterRecovery(node, auth, slurped, jsonDBFile, goldenDB){
    def recoverySlave = null
    def resp = null
    def distList = slurped.securityReplication.urls
    ...
    if (node == 'SLAVE'){
        ...
        if (resp[1] != 200) {
            ...
            if (retryCount >= 3){
                ...
                JsonBuilder builder = new JsonBuilder(slurped)
                jsonDBFile.withWriter{builder.writeTo(it)}
                log.debug("${node}: builder: ${builder.toString()}")
            } else {
...
def bringUp(node, whoami, auth, database, jsonDBFile, goldenDB){
    if (node == 'SLAVE'){
        ...
        //update securityReplication.json file recovery to true
        database.securityReplication.recovery = true
        JsonBuilder builder = new JsonBuilder(database)
        jsonDBFile.withWriter{builder.writeTo(it)}
        log.debug("${node}: builder: ${builder.toString()}")
    } else if (node == 'MASTER'){
        ...
        syncAll(distList, dbMasterPatch, whoami, auth, 'dbSync', database, targetFile)
    } else {
        log.error("ALL: invalid node ${node}")
    }
}
