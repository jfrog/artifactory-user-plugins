...
executions {
    ...
    */
    ...
    -----------------------------------------------------------------------------
    Usage: curl -X GET http://localhost:8081/artifactory/api/plugins/execute/secRepDataGet -d <data>
    -----------------------------------------------------------------------------
    User: Master
    -----------------------------------------------------------------------------
    Input: 
    ...
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        ...
        if (whoami != MASTER) {
            ...
            if (bigDiff.isEmpty()){
                ...
                //TODO: Apply the new merged master golden into the DB
            }
        }
    }
}
...
def remoteCall(baseurl, auth, method, data){
    switch(method) {
        ...
        case 'plugin':
            ...
            break
        case 'data_retrieve':
            def url = "${baseurl}/api/plugins/execute/secRepDataGet"
            req = new HttpGet(url)
            req.addHeader("User-Agent", HttpUtils.artifactoryUserAgent)
...
def sendSlavesGoldenCopy(distList, whoami, auth, mergedPatch){
    ...
    log.debug("MASTER: the merged patch is: ${mergedPatch.getClass()}")
...
def grabStuffFromSlaves(distList, masterDiff, whoami, auth){
    ...
    log.debug("MASTER: masterDiff is ${masterDiff}")
    ...
    for (i = 0; i < distList.size(); i++) {
        ...
        if (MASTER == instance && MASTER == whoami){
            ...
            if ((newDiff.substring(indexOfOpenBracket+1, indexOfLastBracket))) {
