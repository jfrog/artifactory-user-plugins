...
jobs {
    securityReplicationWorker(cron: "*/30 * * * * ?") {
        MASTER = null
        DOWN_INSTANCE = []
        def newDiffFile = new JsonBuilder()
        def bigDiffFile = []
        ...
        if (whoami != MASTER) {
            ...
            def bigDiff = grabStuffFromSlaves(distList, slurped2, whoami, auth)
            ...
            sendSlavesGoldenCopy(distList, whoami, auth, bigDiff)
        }
    }
}
...
def sendSlavesGoldenCopy(distList, whoami, auth, bigDiff){
    ...
    for (i = 0; i < distList.size(); i++){
        ...
        if ((MASTER == instance) && (MASTER == whoami)) {
            ...
            log.debug("MASTER: Sent Data, Slave responds: ${response}")
        }
    }
}
...
def grabStuffFromSlaves(distList, slurped2, whoami, auth){
    def bigDiff = []
    bigDiff << slurped2.toString()
